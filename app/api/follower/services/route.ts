...
import { createServiceOnFollower } from "@/lib/serviceUtils";
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = api.follower.services.create.bodySchema!.safeParse(jsonRaw);
    if (!parsed.success) {
    ...
    const service = await createServiceOnFollower(config);
